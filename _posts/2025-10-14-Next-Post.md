...
```matlab
% --- Load vibrometer FRFs ---
load('../data/FRF_reconstruction_results.mat','results');
freq = results.freq;
H_ref  = results.H_ref;
H_geom = results.H_geom;

% --- Load tri-axial accelerometer FRF ---
accData = load('../data/accel_point.mat');   % adjust filename
H_acc_xyz = accData.accel.FRF.y_values.values; % Nx3 complex array
freq_acc  = transpose((accData.accel.FRF.x_values.start_value: ...
    accData.accel.FRF.x_values.number_of_values-1) * ...
    accData.accel.FRF.x_values.increment);

% Interpolate accelerometer Z channel
H_acc_z = interp1(freq_acc, H_acc_xyz(:,3), freq, 'pchip');
